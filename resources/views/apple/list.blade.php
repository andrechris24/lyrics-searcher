		...
		@foreach ($results as $result)
			@php
				$length = gmdate(
					'i:s',
					round(
						($result['trackTimeMillis'] ?? $result['durationInMillis']) / 1000,
						0,
						PHP_ROUND_HALF_UP
					)
				);
				$art =
					$result['artworkUrl600']??$result['artworkUrl100'] ??
					($result['artworkUrl60'] ??
						($result['artworkUrl30'] ??
							($result['artwork']['url'] ??
								'https://placehold.co/500?text=' .
									urlencode($result['collectionName'] ?? $result['albumName'] ?? $result['trackName'] ?? $result['name']))));
			@endphp
			<div class="col">
				<div class="card">
					<img src="{{ $art }}" class="card-img-top"
						alt="{{ $result['trackName'] ?? $result['name'] }}">
					...
					<div class="card-footer">
						<div class="btn-group" role="group">
							...
							<button type="button" class="btn btn-secondary download-btn"
								@disabled(
									(empty($result['trackViewUrl']) && empty($result['url'])) ||
										empty(env('PAXSENIX_TOKEN'))
								)
								data-href="{{ $result['trackViewUrl'] ?? $result['url'] }}"
								data-artist="{{ $result['artistName'] }}"
								data-title="{{ $result['trackName'] ?? $result['name'] }}"
								data-coreui-toggle="tooltip" data-coreui-title="Download song (Buggy server)">
								<i class="fa-solid fa-download"></i>
							</button>
							<a href="{{ $result['trackViewUrl'] ?? ($result['url'] ?? '#') }}"
								@class([
									'btn',
									'btn-success',
									'disabled' => empty($result['trackViewUrl']) && empty($result['url'])
								])
								aria-disabled="{{ empty($result['trackViewUrl']) && empty($result['url']) }}"
								target="_blank" data-coreui-toggle="tooltip"
								data-coreui-title="Go to Apple Music">
