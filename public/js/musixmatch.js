...
syncedDL.onclick = function (e) {
	...
	$.ajax({
		...
		success: function (data) {
			const blob = new Blob([syncedContents+data.content],{type: 'text/plain', charset: 'utf-8'});
...
richsyncDL.onclick = function (e) {
	...
	if(confirmDL){
		$.ajax({
			...
			success: function (data) {
				const blob = new Blob([richsyncContents+data.content],{type: 'text/plain', charset: 'utf-8'});
				let url = window.URL.createObjectURL(blob);
				let a = document.createElement("a");
				a.href = url;
				a.download = fileName + ".lrc"
