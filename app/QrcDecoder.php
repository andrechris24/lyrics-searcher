...
class QrcDecoder
{
	...
	{
		//Source: https://github.com/NanoRocky/meting-api/blob/master/src/QrcDecode.php
	}
	private $en_mode = 1;
	private $de_mode = 0;
	...
	private function f(int $state, array $key): int
	{
		...
		$state =
			...
			($this->sbox[1][$this->sbox_bit((($lrgstate[0] & 0x03) << 4) | ($lrgstate[1] >> 4))] <<
				24) |
			($this->sbox[2][$this->sbox_bit((($lrgstate[1] & 0x0f) << 2) | ($lrgstate[2] >> 6))] <<
				20) |
			($this->sbox[3][$this->sbox_bit($lrgstate[2] & 0x3f)] << 16) |
			($this->sbox[4][$this->sbox_bit($lrgstate[3] >> 2)] << 12) |
			($this->sbox[5][$this->sbox_bit((($lrgstate[3] & 0x03) << 4) | ($lrgstate[4] >> 4))] <<
				8) |
			($this->sbox[6][$this->sbox_bit((($lrgstate[4] & 0x0f) << 2) | ($lrgstate[5] >> 6))] <<
				4) |
	...
	public function decode(string $hex): string
	{
		...
		$data = "";
		if (strlen($hex) % 2 === 1) {
			Log::warning('Odd length Hex string: ' . $hex);
			$hex = '0' . $hex;
		}
		$bin = hex2bin($hex);
